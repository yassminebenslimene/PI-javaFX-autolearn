...
public class FrontChapitreController {
    ...
    private Set<Integer> startedIds = new HashSet<>();
    ...
    public void setCours(Cours cours) {
        ...
        labelCourseMeta.setText("Matiere: " + cours.getMatiere()
        ...
        for (Chapitre chapitre : chapitres) {
            ...
            anim.setDelay(Duration.millis(80L * i));
            anim.play();
            i++;
        }
    ...
    private VBox buildChapitreCard(Cours cours, Chapitre chapitre) {
        ...
        if (contenuText.length() > 80) {
            contenuText = contenuText.substring(0, 80) + "...";
        }
        ...
        Label statusBadge = new Label(buildStatusText(chapitre));
        statusBadge.setStyle(buildStatusStyle(chapitre));
        ...
        btnLire.setOnAction(e -> {
            if (onLireChapitre != null) {
                onLireChapitre.accept(cours, chapitre);
            }
        });
        ...
        btnQuiz.setOnAction(e -> {
            if (onPasserQuiz != null) {
                onPasserQuiz.accept(chapitre);
            }
        });

        VBox buttons = new VBox(8, btnLire, btnPdf, btnQuiz);

        VBox card = new VBox(12, iconBox, titre, contenu, badgeBox, statusBox, coursBox, buttons);
        ...
        card.setOnMouseEntered(e -> card.setStyle("-fx-background-color:#f5f3ff; -fx-background-radius:18;"
            + "-fx-border-color:#c4b5fd; -fx-border-radius:18;"
            + "-fx-effect:dropshadow(gaussian,rgba(129,140,248,0.18),16,0,0,5);"));
        card.setOnMouseExited(e -> card.setStyle("-fx-background-color:white; -fx-background-radius:18;"
            + "-fx-border-color:#e2e8f0; -fx-border-radius:18;"
            + "-fx-effect:dropshadow(gaussian,rgba(15,23,42,0.09),14,0,0,4);"));
    ...
    private String buildStatusText(Chapitre chapitre) {
        if (completedIds.contains(chapitre.getId())) {
            return "✓  Termine";
        }
        if (startedIds.contains(chapitre.getId())) {
            return "◔  En cours";
        }
        return "•  A faire";
    }

    private String buildStatusStyle(Chapitre chapitre) {
        if (completedIds.contains(chapitre.getId())) {
            return "-fx-background-color:rgba(5,150,105,0.12);"
                + "-fx-text-fill:#059669; -fx-font-size:11; -fx-font-weight:700;"
                + "-fx-padding:4 12 4 12; -fx-background-radius:999;";
        }
        if (startedIds.contains(chapitre.getId())) {
            return "-fx-background-color:rgba(245,158,11,0.14);"
                + "-fx-text-fill:#d97706; -fx-font-size:11; -fx-font-weight:700;"
                + "-fx-padding:4 12 4 12; -fx-background-radius:999;";
        }
        return "-fx-background-color:rgba(148,163,184,0.14);"
            + "-fx-text-fill:#64748b; -fx-font-size:11; -fx-font-weight:700;"
            + "-fx-padding:4 12 4 12; -fx-background-radius:999;";
    }
    ...
    private void telechargerPdf(Cours cours, Chapitre chapitre) {
        ...
        try {
            ...
            doc.add(new Paragraph("Matiere : " + cours.getMatiere() + "   |   Niveau : " + cours.getNiveau()
                + "   |   Duree : " + cours.getDuree() + "h", fontLabel));
            doc.add(new Paragraph(" "));
            ...
            doc.add(new Paragraph(" "));
            doc.add(new com.itextpdf.text.Chunk(line));
            Paragraph footer = new Paragraph("Genere par AutoLearn", fontTitreCours);
            ...
            alert.setContentText("Le chapitre a ete sauvegarde :\n" + file.getAbsolutePath());
            alert.showAndWait();
        } catch (Exception ex) {
            ex.printStackTrace();
            ...
            alert.setHeaderText(null);
            alert.setContentText("Impossible de generer le PDF : " + ex.getMessage());
            alert.showAndWait();
        }
    }
}
