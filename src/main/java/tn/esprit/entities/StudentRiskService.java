...
public class StudentRiskService {
     ...
     * - CTE (WITH)
     * - Agrégations multiples (COUNT, AVG, SUM)
     * - Sous-requêtes
     * - Gestion des NULL (COALESCE, NULLIF)
     * - DATEDIFF pour calculer l'inactivité
    ...
    public List<StudentRisk> getStudentsAtRisk() {
        ...
        try (Statement stmt = connection.createStatement();
            ...
            while (rs.next()) {
                StudentRisk student = new StudentRisk();
                student.setUserId(rs.getInt("userId"));
                student.setPrenom(rs.getString("prenom"));
                student.setNom(rs.getString("nom"));
                student.setNiveau(rs.getString("niveau"));
                student.setTotalTentatives(rs.getInt("total_tentatives"));
                student.setChallengesTermines(rs.getInt("challenges_termines"));
                student.setScoreMoyen(rs.getDouble("score_moyen"));

                Timestamp ts = rs.getTimestamp("derniere_activite");
                if (ts != null) {
                    student.setDerniereActivite(ts.toLocalDateTime());
                }

                student.setJoursInactivite(rs.getLong("jours_inactivite"));
                student.setChallengesAbandonnes(rs.getInt("challenges_abandonnes"));
                student.setStatutRisque(rs.getString("statut_risque"));
                student.setTauxAbandon(rs.getDouble("taux_abandon"));
    ...
    public int getAttentionCount() {
        String sql = """
            ...
            WHERE jours_inactivite > 14 AND score_moyen < 40
        """;
