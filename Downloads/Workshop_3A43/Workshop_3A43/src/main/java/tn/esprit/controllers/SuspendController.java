...
public class SuspendController {
    ...
    public void setUser(User user) {
        this.user = user;
        labelSubtitle.setText(user.getPrenom() + " " + user.getNom() + " ÔÇö " + user.getEmail());

        if (user.isIsSuspended()) {
            labelTitle.setText("Lever la suspension");
            labelCurrentStatus.setText("Statut actuel : Ôøö Suspendu");
            labelCurrentStatus.setStyle("-fx-text-fill:#f87171; -fx-font-weight:bold;");
            btnAction.setText("Ô£ö  Lever la suspension");
            ...
            comboReason.setPromptText("ÔÇö");
            if (user.getSuspensionReason() != null) comboReason.setValue(user.getSuspensionReason());
        } else {
            labelTitle.setText("Suspendre l'├®tudiant");
            labelCurrentStatus.setText("Statut actuel : Ô£à Actif");
            labelCurrentStatus.setStyle("-fx-text-fill:#4ade80; -fx-font-weight:bold;");
            btnAction.setText("ÔÜá  Suspendre");
    ...
    private void onAction() {
        ...
        if (!user.isIsSuspended()) {
            ...
            if (selected == null || selected.isBlank()) {
                errorReason.setText("Veuillez s├®lectionner une raison.");
                return;
            }
            String reason = AUTRE.equals(selected)
                    ...
                    : selected;
            if (reason.isBlank()) {
                errorReason.setText("Veuillez pr├®ciser la raison.");
                return;
            }
