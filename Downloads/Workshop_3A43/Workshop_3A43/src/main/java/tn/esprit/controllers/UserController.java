...
public class UserController {
    ...
    // ÔöÇÔöÇ Index ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    // ÔöÇÔöÇ Form ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    private void initTable() {
        ...
        // Dark scrollbar + dark header ÔÇö applied once skin is ready
        ...
        // Force dark row background ÔÇö overrides JavaFX default white
        ...
        // ÔöÇÔöÇ Level badge ÔöÇÔöÇ
        colNiveau.setCellValueFactory(data -> {
            User u = data.getValue();
            return new SimpleStringProperty((u instanceof Etudiant e && e.getNiveau() != null) ? e.getNiveau() : "ÔÇö");
        });
        colNiveau.setCellFactory(col -> new TableCell<>() {
            @Override protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null || "ÔÇö".equals(item)) { setText("ÔÇö"); setGraphic(null); return; }
        ...
        // ÔöÇÔöÇ Joined date ÔöÇÔöÇ
        colCreated.setCellValueFactory(data -> {
            User u = data.getValue();
            return new SimpleStringProperty(u.getCreatedAt() != null ? SDF.format(u.getCreatedAt()) : "ÔÇö");
        });
    ...
    @FXML private void onNewUser() {
        if (!SessionManager.isAdmin()) { showAlert(Alert.AlertType.WARNING, "Acc├¿s refus├®", "R├®serv├® aux administrateurs."); return; }
        openFormWindow(null);
    }
    ...
    private void onSuspendUser(User sel) {
        ...
        if (!(sel instanceof tn.esprit.entities.Etudiant)) {
            showAlert(Alert.AlertType.WARNING, "Action impossible", "Seul un ├®tudiant peut ├¬tre suspendu."); return;
        }
    ...
    private void openDetailWindow(User user) {
        try {
            ...
            stage.setTitle("D├®tails ÔÇö " + user.getPrenom() + " " + user.getNom());
    ...
    private void openSuspendWindow(User user) {
        try {
            ...
            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.setTitle(user.isIsSuspended() ? "Lever la suspension" : "Suspendre ÔÇö " + user.getPrenom());
            stage.setResizable(false);
    ...
    private void openFormWindow(User user) {
        try {
            ...
            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.setTitle(user == null ? "Nouvel utilisateur" : "Modifier ÔÇö " + user.getPrenom() + " " + user.getNom());
            stage.setResizable(false);
    ...
    @FXML private void onDeleteUser() {
        showAlert(Alert.AlertType.WARNING, "Action non autoris├®e", "L'administrateur ne peut pas supprimer un utilisateur. Utilisez la suspension.");
    }
    ...
    private void initForm() {
        // Admin can only create students ÔÇö no role selection needed
    ...
    public void setEditingUser(User user) {
        ...
        if (isEditMode) {
            formTitle.setText("Modifier ├®tudiant");
            ...
            formTitle.setText("Nouvel ├®tudiant");
    ...
    @FXML private void onSave() {
        ...
        if (!isEditMode) {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Succ├¿s", "├ëtudiant cr├®├® avec succ├¿s.");
        } else {
    ...
    private boolean validateForm() {
        ...
        else if (nom.length() < 2 || nom.length() > 50) { errorNom.setText("Entre 2 et 50 caract├¿res"); valid = false; }
        else if (!nom.matches("^[a-zA-Z├Ç-├┐\\s'-]+$")) { errorNom.setText("Lettres uniquement"); valid = false; }

        if (prenom.isEmpty()) { errorPrenom.setText("Le pr├®nom est obligatoire"); valid = false; }
        else if (prenom.length() < 2 || prenom.length() > 50) { errorPrenom.setText("Entre 2 et 50 caract├¿res"); valid = false; }
        else if (!prenom.matches("^[a-zA-Z├Ç-├┐\\s'-]+$")) { errorPrenom.setText("Lettres uniquement"); valid = false; }
        ...
        if (!isEditMode) {
            ...
            else if (password.length() < 6) { errorPassword.setText("Minimum 6 caract├¿res"); valid = false; }
            else if (!password.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&]).+$")) {
                errorPassword.setText("Maj + min + chiffre + sp├®cial requis"); valid = false;
            }
        } else if (!password.isEmpty()) {
            if (password.length() < 6) { errorPassword.setText("Minimum 6 caract├¿res"); valid = false; }
            else if (!password.matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&]).+$")) {
                errorPassword.setText("Maj + min + chiffre + sp├®cial requis"); valid = false;
            }
        }
