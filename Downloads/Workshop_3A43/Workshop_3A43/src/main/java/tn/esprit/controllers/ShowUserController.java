...
public class ShowUserController {
    ...
    public void setUser(User user) {
        ...
        valCreated.setText(user.getCreatedAt() != null ? SDF.format(user.getCreatedAt()) : "ÔÇö");

        boolean isEtudiant = user instanceof Etudiant;
        rowNiveau.setVisible(isEtudiant); rowNiveau.setManaged(isEtudiant);
        if (isEtudiant) valNiveau.setText(((Etudiant) user).getNiveau() != null ? ((Etudiant) user).getNiveau() : "ÔÇö");

        if (user.isIsSuspended()) {
            valStatut.setText("Ôøö  Suspendu");
            valStatut.setStyle("-fx-text-fill:#f87171; -fx-font-weight:bold;");
            rowSuspension.setVisible(true); rowSuspension.setManaged(true);
            valSuspendedAt.setText(user.getSuspendedAt() != null ? SDF.format(user.getSuspendedAt()) : "ÔÇö");
            valSuspendedReason.setText(user.getSuspensionReason() != null ? user.getSuspensionReason() : "ÔÇö");
        } else {
            ...
            if (recentLogin) {
                valStatut.setText("Ô£à  Actif  ÔÇó  Connect├® r├®cemment");
                valStatut.setStyle("-fx-text-fill:#4ade80; -fx-font-weight:bold;");
            } else {
                valStatut.setText("Ô£à  Actif");
                valStatut.setStyle("-fx-text-fill:#4ade80; -fx-font-weight:bold;");
            }
