...
public class ChapitreController {
    ...
    private void initTable() {
        ...
        colType.setCellFactory(col -> new TableCell<>() {
            @Override protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null || "-".equals(item)) { setText("ÔÇö"); setGraphic(null); return; }
    ...
    private boolean validateForm() {
        ...
        // Titre : 3 ├á 100 caract├¿res
        String titre = fieldTitre.getText().trim();
        if (titre.isEmpty()) {
            errorTitre.setText("Le titre est obligatoire");
            valid = false;
        } else if (titre.length() < 3) {
            errorTitre.setText("Le titre doit contenir au moins 3 caract├¿res");
            valid = false;
        } else if (titre.length() > 100) {
            errorTitre.setText("Le titre ne peut pas d├®passer 100 caract├¿res");
            valid = false;
        } else if (!titre.matches("^[\\p{L}0-9 \\-_.,:'\"()]+$")) {
            errorTitre.setText("Caract├¿res sp├®ciaux non autoris├®s dans le titre");
            valid = false;
        }

        // Contenu : minimum 10 caract├¿res pour ├¬tre significatif
        String contenu = areaContenu.getText().trim();
        if (contenu.isEmpty()) {
            errorContenu.setText("Le contenu est obligatoire");
            valid = false;
        } else if (contenu.length() < 10) {
            errorContenu.setText("Le contenu doit contenir au moins 10 caract├¿res");
            valid = false;
        } else if (contenu.length() > 1000) {
            errorContenu.setText("Le contenu ne peut pas d├®passer 1000 caract├¿res");
            valid = false;
        }
        ...
        try {
            int ordre = Integer.parseInt(fieldOrdre.getText().trim());
            if (ordre < 1) {
                errorOrdre.setText("L'ordre doit ├¬tre au moins 1");
                valid = false;
            } else if (ordre > 99) {
                errorOrdre.setText("L'ordre ne peut pas d├®passer 99");
                valid = false;
            }
        } catch (NumberFormatException e) {
            errorOrdre.setText("L'ordre doit ├¬tre un nombre entier (ex: 1)");
            valid = false;
        }

        // Ressource URL : si renseign├®e, doit commencer par http:// ou https://
        ...
        // Fichier : si renseign├®, doit avoir une extension reconnue
        String fichier = fieldRessourceFichier.getText().trim();
