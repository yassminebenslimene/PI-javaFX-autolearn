...
public class UserService implements IUserService {
    ...
    public void ajouter(User user) {
        ...
        if (user instanceof Etudiant e && (e.getNiveau() == null || e.getNiveau().isBlank())) {
            System.err.println("Niveau obligatoire pour ├®tudiant."); return;
        }
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            System.out.println("Ô£ö Ajout├® ID=" + user.getId());
    ...
    /** Find by email ÔÇö used for login */
    ...
    public void modifier(User updated) {
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ...
            System.out.println(rows > 0 ? "Ô£ö Modifi├®." : "Ô£ÿ ID introuvable.");
    ...
    public void supprimer(int id) {
        ...
        try (PreparedStatement ps = cnx.prepareStatement("DELETE FROM user WHERE userId=?")) {
            ps.setInt(1, id);
            int rows = ps.executeUpdate();
            System.out.println(rows > 0 ? "Ô£ö Supprim├® id=" + id : "Ô£ÿ ID introuvable.");
